...
if ($id <= 0) {
    die(json_encode(['success' => false, 'message' => 'ID richiesta non valido']));
}

// Aggiorno il campo isDeleted invece di eliminare fisicamente il record
$sql = "UPDATE Richieste SET isDeleted = 1 WHERE id = ?";
...
if ($stmt->execute()) {
    if ($stmt->affected_rows > 0) {
        echo json_encode(['success' => true, 'message' => 'Richiesta eliminata con successo']);
    } else {
        echo json_encode(['success' => false, 'message' => 'Nessuna richiesta trovata con questo ID']);
    }
} else {
